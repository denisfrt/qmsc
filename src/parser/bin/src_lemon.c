...
static char *generatedPrefix = NULL;
static int gencpp = 0;
static void handle_P_option(char *z)
{
    generatedPrefix = (char *) lemon_malloc(lemonStrlen(z) + 1);
    if (generatedPrefix == 0) {
        fprintf(stderr, "out of memory\n");
        exit(1);
    }
    lemon_strcpy(generatedPrefix, z);
}
...
int main(int argc, char **argv){
  ...
  static struct s_options options[] = {
    ...
    {OPT_FLAG, "C", (char*)&gencpp, "Generate .cpp file."},
    ...
    {OPT_FSTR, "P", (char*)&handle_P_option, "Generated files added prefix."},
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix)
{
  ...
  if (generatedPrefix) {
      sz += lemonStrlen(generatedPrefix);
  }
  ...
  if (generatedPrefix) {
      lemon_strcat(name, generatedPrefix);
  }
...
void ReportTable(
  ...
  out = file_open(lemp, gencpp ? ".cpp" : ".c", "wb");
  if( out==0 ){
    fclose(in);
    return;
  }
